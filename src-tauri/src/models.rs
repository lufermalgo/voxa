...
impl ModelManager {
    ...
    pub fn get_llama_path(&self) -> PathBuf {
        self.base_path.join("smollm2-1.7b-instruct-q4_k_m.gguf")
    }
...
pub async fn get_models_info(app_handle: AppHandle) -> Result<ModelsStateInfo, String> {
    ...
    let models_meta = vec![
        ...
        ("SmolLM2-1.7B (Refinement)", "smollm2-1.7b-instruct-q4_k_m.gguf"),
    ];
...
mod tests {
    ...
    fn test_model_paths() {
        ...
        assert!(llama_path.to_str().unwrap().contains("smollm2-1.7b-instruct"));
    }
}
