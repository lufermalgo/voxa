...
export function SettingsPanel({ initialTab = "general", uiLocale }: SettingsPanelProps) {
  ...
  if (loading || !settings) return (
    <div className="h-full w-full flex items-center justify-center bg-background">
      <div className="text-white/60 font-black tracking-[0.4em] text-[10px] animate-pulse uppercase">{t.loading}</div>
    </div>
  );
  ...
  return (
    <div className="h-screen w-screen bg-background flex flex-col overflow-hidden select-none">
      ...
      <header className="h-12 flex items-center justify-between px-8 border-b border-white/[0.10] flex-shrink-0 bg-white/[0.02]">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-sm font-black text-white/90 font-headline leading-none tracking-tight">Voxa</span>
            <span className="text-[9px] font-semibold text-white/40 uppercase tracking-[0.2em] hidden sm:inline">{t.app_subtitle}</span>
          </div>
        </div>
        <span className="text-[9px] font-mono text-white/50 tracking-widest">v{appVersion}</span>
      </header>

      <div className="flex-1 flex overflow-hidden">
        {/* ── SIDEBAR ── 200px, barra izquierda activa */}
        <aside className="w-[200px] flex flex-col py-6 border-r border-white/[0.10] flex-shrink-0 bg-white/[0.02]">
          <nav className="flex-1 px-3 space-y-0.5">
            {tabs.map(tab => (
              ...
              >
                ...
                <span className={`material-symbols-outlined text-[18px] ${activeTab === tab.id ? 'material-symbols-fill text-primary' : 'text-white/50'}`}>
                  {tab.icon}
                </span>
          ...
          <div className="px-4 pb-2">
            <p className="text-[9px] text-white/40 leading-relaxed">
              <span className="text-white/60 font-bold">Cmd+L</span>{' '}
          ...
          <div className="animate-in fade-in slide-in-from-right-1 duration-200">
            ...
            {activeTab === "history" && (
              <section>
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-baseline gap-2">
                    <h3 className="text-lg font-black text-white font-headline">{t.voice_history}</h3>
                    {transcripts.length > 0 && (
                      <span className="text-[10px] text-white/45 font-mono">({transcripts.length})</span>
                    )}
                  </div>
                  {transcripts.length > 0 && (
                    <button onClick={clearHistory} className="text-[9px] font-semibold uppercase tracking-widest text-error/70 hover:text-error/90 transition-colors py-1">
                      {t.clear_history}
                    </button>
                  )}
                </div>
                ...
                {transcripts.length === 0 ? (
                  <div className="py-16 flex flex-col items-center gap-3">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/40">{t.no_transcripts}</p>
                  </div>
                ) : (
                  <div>
                    {(() => {
                      ...
                      return groups.map((group, gi) => (
                        <div key={group.label} className={gi > 0 ? 'mt-6' : ''}>
                          <div className="flex items-center justify-between py-2 border-t border-white/[0.10]">
                            <span className="text-[9px] font-bold uppercase tracking-[0.3em] text-primary/70">{group.label}</span>
                          </div>
                          {group.items.map((transcript) => {
                            ...
                            return (
                              <div key={transcript.id}>
                                {isEditing ? (
                                  <div className="py-3 border-b border-white/[0.08] space-y-3">
                                    <textarea
                                      className="w-full text-sm text-on-surface leading-relaxed font-medium bg-white/[0.08] rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-primary/30 resize-none"
                                    ...
                                    <div className="flex gap-2">
                                      ...
                                      <button onClick={() => { setEditingTranscriptId(null); setLearnedWords([]); }} className="px-4 bg-white/[0.08] text-white/65 py-2 rounded-lg text-[10px] font-black uppercase tracking-wider hover:bg-white/[0.12] transition-all">{t.cancel}</button>
                                    </div>
                                  </div>
                                ) : (
                                  <div className="group flex items-center gap-3 py-2.5 border-b border-white/[0.08] last:border-0 hover:bg-white/[0.08] transition-colors rounded-sm -mx-2 px-2">
                                    <span className="font-mono text-[10px] text-white/55 flex-shrink-0 w-10">{time}</span>
                                    <p className={`flex-1 text-sm text-white/90 font-medium min-w-0 cursor-pointer ${isExpanded ? 'whitespace-pre-wrap' : 'truncate'}`} onClick={() => setExpandedTranscriptId(isExpanded ? null : transcript.id)}>{transcript.content}</p>
                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                                      <CopyButton text={transcript.content} copyLabel={t.copy_text} />
                                      <button onClick={() => startEditTranscript(transcript.id, transcript.content)} className="p-1 rounded text-white/55 hover:text-primary transition-colors"><span className="material-symbols-outlined text-[14px]">edit</span></button>
                                      <button onClick={() => deleteTranscript(transcript.id)} className="p-1 rounded text-white/55 hover:text-error transition-colors"><span className="material-symbols-outlined text-[14px]">close</span></button>
                                    </div>
                                  </div>
                                )}
                              </div>
            ...
            {activeTab === "profiles" && (
              <section>
                <div className="flex items-baseline gap-2 mb-6">
                  <h3 className="text-lg font-black text-white font-headline">{t.transformation_profiles}</h3>
                </div>
                <div>
                  {profiles.map(profile => {
                    ...
                    return (
                      <div key={profile.id}>
                        <div onClick={() => updateSetting("active_profile_id", profile.id.toString())} className="group flex items-center gap-3 py-2.5 border-b border-white/[0.08] last:border-0 cursor-pointer hover:bg-white/[0.08] transition-colors rounded-sm -mx-2 px-2">
                          ...
                          <span className={`text-xs font-black uppercase tracking-widest flex-shrink-0 w-28 ${isActive ? 'text-primary' : 'text-white/65'}`}>{profile.name}</span>
                          <span className="flex-1 text-[11px] italic text-white/50 truncate min-w-0">{profile.system_prompt || t.exact_transcription}</span>
                          <button onClick={(e) => { e.stopPropagation(); if (editingProfileId === profile.id) { setEditingProfileId(null); } else { setEditingProfileId(profile.id); setEditName(profile.name); setEditPrompt(profile.system_prompt); setEditIcon(profile.icon || 'psychology'); setIsCreatingProfile(false); } }} className="opacity-0 group-hover:opacity-100 p-1 rounded text-white/65 hover:text-white/90 transition-all flex-shrink-0">
                            <span className="material-symbols-outlined text-[14px]">edit</span>
                          </button>
                        </div>
                        {editingProfileId === profile.id && (
                          <div className="ml-6 mt-1 mb-3 p-5 rounded-xl bg-white/[0.10] backdrop-blur-xl space-y-4 animate-in slide-in-from-top-2 duration-200">
                            <div className="grid grid-cols-2 gap-3">
                              <div className="space-y-1.5">
                                <label className="text-[9px] font-black text-white/60 uppercase tracking-widest">{t.name_label}</label>
                              ...
                              <div className="space-y-1.5">
                                <label className="text-[9px] font-black text-white/60 uppercase tracking-widest">{t.icon_label}</label>
                                <div className="flex flex-wrap gap-1.5 p-2 bg-primary/5 rounded-lg max-h-[80px] overflow-y-auto custom-scrollbar">
                                  {AVAILABLE_ICONS.map(icon => (
                                    <button key={icon} onClick={() => setEditIcon(icon)} className={`p-1.5 rounded-lg transition-all ${editIcon === icon ? 'bg-primary text-on-primary' : 'text-white/60 hover:text-white/90 hover:bg-white/[0.08]'}`}>
                            ...
                            <div className="space-y-1.5">
                              <label className="text-[9px] font-black text-white/60 uppercase tracking-widest">{t.prompt_label}</label>
                            ...
                            <div className="space-y-1.5">
                              <label className="text-[9px] font-black text-white/60 uppercase tracking-widest">{t.formatting_mode_label}</label>
                              <div className="flex gap-2">
                                {(['plain', 'markdown'] as const).map(mode => {
                                  const isModeActive = (profile.formatting_mode || 'plain') === mode;
                                  return (
                                    <button key={mode} onClick={() => updateProfileFormattingMode(profile.id, mode)} className={`flex-1 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all ${isModeActive ? 'bg-primary text-background' : 'bg-background/40 text-white/65 hover:bg-white/[0.08]'}`}>
                                      <div>{mode === 'plain' ? t.formatting_mode_plain : t.formatting_mode_markdown}</div>
                                      <div className={`text-[8px] normal-case tracking-normal mt-0.5 font-normal ${isModeActive ? 'text-on-primary/70' : 'text-white/60'}`}>{mode === 'plain' ? t.formatting_mode_plain_desc : t.formatting_mode_markdown_desc}</div>
                                    </button>
                                  );
                                })}
                              </div>
                            ...
                            <div className="flex gap-2 pt-1">
                              ...
                              <button onClick={() => setEditingProfileId(null)} className="px-4 bg-white/[0.08] text-white/65 py-2 rounded-lg text-[10px] font-black uppercase tracking-wider hover:bg-white/[0.12] transition-all">{t.cancel}</button>
                            </div>
                          </div>
                        )}
                      </div>
                  ...
                  {!isCreatingProfile ? (
                    <button onClick={() => { setIsCreatingProfile(true); setEditingProfileId(null); setNewName(""); setNewPrompt(""); setNewIcon("psychology"); }} className="mt-3 flex items-center gap-2 text-[11px] font-black uppercase tracking-wider text-white/55 hover:text-primary transition-colors py-2">
                    ...
                    <div className="mt-3 p-5 rounded-xl bg-primary/5 space-y-4 animate-in zoom-in-95 duration-200">
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="flex flex-wrap gap-1.5 p-2 bg-primary/5 rounded-lg max-h-[80px] overflow-y-auto custom-scrollbar">
                            {AVAILABLE_ICONS.map(icon => (
                              <button key={icon} onClick={() => setNewIcon(icon)} className={`p-1.5 rounded-lg transition-all ${newIcon === icon ? 'bg-primary text-on-primary' : 'text-white/60 hover:text-white/90 hover:bg-white/[0.08]'}`}>
                                <span className="material-symbols-outlined text-[16px]">{icon}</span>
                              </button>
                            ))}
                          </div>
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-black text-white/60 uppercase tracking-widest">{t.instructions_custom}</label>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => setIsCreatingProfile(false)} className="px-4 bg-white/[0.08] text-white/65 py-2 rounded-lg text-[10px] font-black uppercase tracking-wider hover:bg-white/[0.12] transition-all">{t.discard}</button>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {activeTab === "dictionary" && (
              <section className="flex flex-col">
                <div className="flex items-baseline gap-2 mb-6">
                  <h3 className="text-lg font-black text-white font-headline">{t.personal_dictionary}</h3>
                  {dictionaryEntries.length > 0 && <span className="text-[10px] text-white/45 font-mono">({dictionaryEntries.length})</span>}
                </div>
                <div className="flex items-center gap-4 pb-2 border-b border-white/[0.10]">
                  <span className="flex-1 text-[9px] font-black uppercase tracking-[0.3em] text-white/40">{t.word ?? "Word"}</span>
                  <span className="flex-1 text-[9px] font-black uppercase tracking-[0.3em] text-white/40">{t.replacement ?? "Replacement"}</span>
                  <span className="w-10 text-right text-[9px] font-black uppercase tracking-[0.3em] text-white/40">{t.usage ?? "Uses"}</span>
                  <span className="w-4" />
                </div>
                {dictionaryEntries.length === 0 ? (
                  <div className="py-10 text-center"><p className="text-[10px] text-white/40 italic font-black uppercase tracking-[0.2em]">{t.dictionary_empty}</p></div>
                ) : (
                  <div>
                    {dictionaryEntries.map(entry => (
                      <div key={entry.word} className="group flex items-center gap-4 py-2 border-b border-white/[0.08] last:border-0 min-h-[36px]">
                        <span className="flex-1 text-sm font-bold text-white/90">{entry.word}</span>
                        <input type="text" placeholder="—" defaultValue={entry.replacement_word ?? ""} onBlur={(e) => { const val = e.target.value.trim() || null; updateReplacement(entry.word, val); }} onKeyDown={(e) => { if (e.key === 'Enter') (e.target as HTMLInputElement).blur(); }} className="flex-1 bg-transparent text-xs text-white/70 placeholder:text-white/40 focus:outline-none focus:bg-white/[0.08] rounded px-1 py-0.5 transition-colors" />
                        <span className="w-10 text-right">
                          {entry.usage_count > 0 ? <span className="text-[10px] font-black text-primary/90">{entry.usage_count}</span> : <span className="text-white/25 text-[10px]">—</span>}
                        </span>
                        <button onClick={() => removeWord(entry.word)} className="w-4 opacity-0 group-hover:opacity-100 text-white/40 hover:text-error transition-all">
                ...
                <div className="sticky bottom-0 pt-3 pb-1 bg-background/80 backdrop-blur-sm flex gap-2 mt-4">
                  <input type="text" placeholder={t.dictionary_placeholder} value={newWord} onChange={(e) => setNewWord(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && newWord.trim()) { addWord(newWord.trim()); setNewWord(""); } }} className="flex-1 bg-white/[0.08] rounded-lg px-3 py-2 text-sm text-on-surface focus:outline-none focus:ring-1 focus:ring-primary/30 placeholder:text-white/40" />
            ...
            {activeTab === "models" && (
              <section>
                <div className="flex items-baseline gap-2 mb-6">
                  <h3 className="text-lg font-black text-white font-headline">{t.models}</h3>
                </div>
                {modelsInfo && (
                  <div>
                    <div className="text-[9px] font-bold uppercase tracking-[0.3em] text-primary/70 pb-2 border-b border-white/[0.10]">{t.path}</div>
                    <div className="flex items-center justify-between py-2.5 border-b border-white/[0.08]">
                      <span className="font-mono text-[11px] text-white/65 truncate flex-1 mr-3">{modelsInfo.base_path}</span>
                      <div className="flex gap-1 flex-shrink-0">
                        <CopyButton text={modelsInfo.base_path} copyLabel={t.path} />
                        <button onClick={() => invoke("open_models_folder")} className="p-1 rounded text-white/55 hover:text-primary transition-colors"><span className="material-symbols-outlined text-[14px]">folder_open</span></button>
                      </div>
                    </div>
                    <div className="text-[9px] font-bold uppercase tracking-[0.3em] text-primary/70 pt-5 pb-2 mt-1 border-t border-white/[0.10]">{t.ai_models}</div>
                    {modelsInfo.models.map((model: any) => (
                      <div key={model.filename} className="flex items-center gap-4 py-2.5 border-b border-white/[0.08] last:border-0 min-h-[40px]">
                        <span className="flex-1 text-sm font-bold text-white/90">{model.display_name}</span>
                        <span className="font-mono text-[10px] text-white/55">{model.filename}</span>
                        <span className="text-[11px] text-white/60 w-16 text-right">{model.size_mb} {t.size_mb}</span>
                    ...
                    <div className="flex justify-end pt-5">
                      <button onClick={() => setConfirmModal({ type: 'redownload' })} disabled={isDownloadingModels} className="text-[10px] font-black uppercase tracking-wider text-white/65 hover:text-white/90 disabled:opacity-20 disabled:cursor-not-allowed transition-colors py-1.5 px-3 rounded-lg hover:bg-white/[0.10]">
            ...
            {activeTab === "general" && (
              <section>
                <div className="flex items-baseline gap-2 mb-6">
                  <h3 className="text-lg font-black text-white font-headline">{t.system_settings}</h3>
                </div>
                <div className="text-[9px] font-bold uppercase tracking-[0.3em] text-primary/70 pb-2 border-b border-white/[0.10]">{t.audio_source}</div>
                <div className="flex items-center justify-between py-2.5 min-h-[40px] border-b border-white/[0.08]">
                  <span className="text-sm text-white/85">{t.audio_source}</span>
                  <div className="relative flex-shrink-0">
                    <select value={settings.mic_id} onChange={(e) => updateSetting("mic_id", e.target.value)} className="bg-white/[0.10] text-white/90 text-xs font-medium rounded-lg pl-3 pr-7 py-1.5 appearance-none cursor-pointer focus:outline-none focus:ring-1 focus:ring-primary/30 hover:bg-white/[0.13] transition-colors">
                      <option value="auto">{t.auto_detect}</option>
                      {micDevices.map(dev => <option key={dev.id} value={dev.id}>{dev.name}</option>)}
                    </select>
                    <span className="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none text-white/60 material-symbols-outlined text-[14px]">expand_more</span>
                  </div>
                </div>
                <div className="flex items-center justify-between pt-5 pb-2 mt-1 border-t border-white/[0.10]">
                  <span className="text-[9px] font-bold uppercase tracking-[0.3em] text-primary/70">{t.global_shortcut}</span>
                  <button onClick={() => { updateSetting("shortcut_push_to_talk", "Alt+Space"); updateSetting("shortcut_hands_free", "F5"); updateSetting("shortcut_paste", "CommandOrControl+Shift+V"); updateSetting("shortcut_cancel", "Escape"); }} className="text-[9px] font-black uppercase tracking-wider text-white/55 hover:text-white/90 transition-colors">{t.reset_defaults}</button>
                </div>
                {[
                  ...
                  return (
                    <div key={sc.key} className="flex items-center justify-between py-2.5 min-h-[40px] border-b border-white/[0.08]">
                      <span className="text-sm text-white/85">{sc.label}</span>
                      <button onClick={() => setCapturingShortcutFor(sc.key as keyof AppSettings)} className={`font-mono text-sm px-2.5 py-1 rounded-md transition-all ${isCapturing ? 'bg-primary/15 text-primary ring-1 ring-primary/30 animate-pulse' : 'bg-white/[0.09] text-white/90 hover:bg-white/[0.13]'}`}>{displayVal}</button>
                    </div>
                  );
                })}
                <div className="text-[9px] font-bold uppercase tracking-[0.3em] text-primary/70 pt-5 pb-2 mt-1 border-t border-white/[0.10]">Behavior</div>
                <div className="flex items-center justify-between py-2.5 min-h-[40px] border-b border-white/[0.08]">
                  <div>
                    <span className="text-sm text-white/85">{t.auto_detect_profile}</span>
                    <p className="text-[10px] text-white/55 mt-0.5">{t.auto_detect_profile_hint}</p>
                  </div>
                ...
                <div className="flex items-center justify-between py-2.5 min-h-[40px]">
                  <span className="text-sm text-white/85">{t.transcription_input_lang}</span>
                  <div className="flex gap-1.5">
                    {['es', 'en'].map(lang => (
                      <button key={lang} onClick={() => updateSetting("language", lang)} className={`px-3 py-1 rounded-full text-[11px] font-black uppercase tracking-wider transition-all ${settings.language === lang ? 'bg-primary text-background' : 'text-white/60 hover:text-white/90 hover:bg-white/[0.08]'}`}>
      ...
      <footer className="h-8 flex items-center px-8 border-t border-white/[0.08] flex-shrink-0">
        <span className="text-[9px] font-mono text-white/50 tracking-widest">{t.footer_engine.replace("{version}", appVersion)}</span>
      </footer>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="bg-surface-container-low/90 backdrop-blur-2xl rounded-2xl ring-1 ring-white/[0.06] p-6 max-w-xs w-full mx-6 space-y-4 animate-in zoom-in-95 duration-200 shadow-2xl">
            ...
            <div className="flex gap-2 justify-end pt-1">
              <button onClick={() => setConfirmModal(null)} className="px-4 py-2 rounded-lg bg-white/[0.10] text-white/65 text-[10px] font-black uppercase tracking-wider hover:bg-white/[0.10] transition-colors">{t.cancel}</button>
...
function CopyButton({ text, copyLabel }: { text: string; copyLabel: string }) {
  ...
  return (
    <button 
      onClick={handleCopy}
      className={`p-2 rounded-lg transition-all ${copied ? 'bg-primary/10 text-primary' : 'bg-white/[0.06] text-white/55 hover:text-white/90 hover:bg-white/[0.10]'}`}
      title={copyLabel}
    >
